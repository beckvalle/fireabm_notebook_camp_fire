...
def normalize_edge_attribute(g, attr, att_min, att_max, new_name):
    for e in g.edges(keys=True, data=True):
        if attr in e[3]:
            # add small value to prevent devision by zero 11-11-24
            e[3][new_name] = (e[3][attr]-att_min)/((att_max-att_min)+0.00001)
        else:
            e[3][new_name] = None 
    return g
...
class NetABM():
    ...
    def run(self, nsteps=None, mutate_rate=0.005, update_interval=100, save_args=None, opt_interval=35, strategies=['dist', 'dist+speed'], opt_reps=1, mix_strat=True, congest_time=25):
        ...
        if save_args is None or len(save_args) < 3:
            ...
            def update(frame_number):
                #print('.', end="")
                if frame_number == 1: #catch duplicate frame number - not sure why this happens
                    self.one_flag.append(frame_number)
                #print('frame_number', frame_number)
                if self.isFinished:
                    print('\tfinished at frame_number', frame_number)
                    return
 # !!! reset closed roads, need to confirm no removal
                ...
                if len(self.one_flag) == 2:#catch duplicate frame number - not sure why this happens
                    self.one_flag = [] 
